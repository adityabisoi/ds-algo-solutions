...
int libraryFine(int d1, int m1, int y1, int d2, int m2, int y2) {
    ...
    if(y1!=y2){
        if(y1>y2)
            fine += (y1-y2)*10000;//no of years late fine.
        else
            fine=0;//returned before due year.
    }   
    else if(m1!=m2){
        if(m1>m2)
            fine += (m1-m2)*500;//number of months late but returned same year.
        else
            fine=0;//returned before due month.
    }
    else if(d1>d2)
        fine+= (d1-d2)*15;//number of days late but returned within same month.
